...
to create-plane
  ...
  ; An airplane is created in the patch -16 -16
...
to take-off
  if length waiting-planes > 0[
    ifelse takeoff-time = 0 and runway-free [
      ifelse LIFO [;and [waiting-time-runway] of turtle first waiting-planes < 500 [
        ask turtle last waiting-planes [move-to patch 16 -12]
        set waiting-planes remove last waiting-planes waiting-planes
      ]
      ...
      ifelse landing < p [set takeoff-time landing-time] [set takeoff-time 0]
    ]
    [if runway-free [set takeoff-time takeoff-time - 1]]]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [xcor = 16 and ycor = -13 ]"
